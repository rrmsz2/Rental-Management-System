            ...
            <div className="text-right hidden sm:block bg-slate-50 px-3 py-2 rounded-lg">
              <div className="flex items-center gap-2 mb-1">
                ...
                {user?.is_manager && !user?.role && (
                  <span className="px-2 py-0.5 rounded-full text-xs bg-cyan-100 text-cyan-700 font-semibold">مدير</span>
                )}
              </div>
              {user?.full_name && user.full_name !== user.phone && (
                <p className="text-sm font-bold text-slate-800">
                  {user.full_name}
                </p>
              )}
              <p className="text-xs text-slate-600" data-testid="user-phone">
                {user?.phone}
              </p>
            </div>
            <Button
